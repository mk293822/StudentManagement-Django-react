...
function TeacherInfoBackend() {
        ...
        axios.get(Api)
            .then(response => {
                ...
                function searchItemCheck() {
                    if (isNaN(searchItem)) {
                        const isName = response.data.filter(data => data.name.toLowerCase().includes(searchItem.toLowerCase()));
                        const isSubject = response.data.filter(data => data.main_subject.toLowerCase().includes(searchItem.toLowerCase()));

                        return searchItem ? isName.length > 0 ? isName : isSubject.length > 0 ? isSubject : [] : response.data;
    ...
    }, [title, postTeacherApiData.post, updateTeacher.update, deleteTeacher.delete, searchItem]);
